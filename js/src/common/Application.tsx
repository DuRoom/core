...
import RequestError, { InternalDuRoomRequestOptions } from './utils/RequestError';
...
export type DuRoomScreens = 'phone' | 'tablet' | 'desktop' | 'desktop-hd';

export type DuRoomGenericRoute = RouteItem<any, any, any>;

export interface DuRoomRequestOptions<ResponseType> extends Omit<Mithril.RequestOptions<ResponseType>, 'extract'> {
  errorHandler?: (error: RequestError) => void;
  url: string;
  // TODO: [DuRoom 2.0] Remove deprecated option
  /**
...
> = {
   ...
   *
   * @see https://duroom.js.org/docs/extend/frontend-pages.html#route-resolvers-advanced
   */
  path: `/${string}`;
} & (
  | {
...
export default class Application {
   ...
   */
  routes: Record<string, DuRoomGenericRoute> = {};
   ...
   */
  screen(): DuRoomScreens {
    const styles = getComputedStyle(document.documentElement);
    return styles.getPropertyValue('--duroom-screen') as ReturnType<Application['screen']>;
  }
  ...
  protected transformRequestOptions<ResponseType>(duroomOptions: DuRoomRequestOptions<ResponseType>): InternalDuRoomRequestOptions<ResponseType> {
    const { background, deserialize, extract, modifyText, ...tmpOptions } = { ...duroomOptions };
    ...
    const options: InternalDuRoomRequestOptions<ResponseType> = {
   ...
   */
  request<ResponseType>(originalOptions: DuRoomRequestOptions<ResponseType>): Promise<ResponseType> {
   ...
   */
  protected requestErrorCatch<ResponseType>(error: RequestError, customErrorHandler: DuRoomRequestOptions<ResponseType>['errorHandler']) {
