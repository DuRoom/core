...
class ViewTest extends TestCase
{
    ...
    public function can_add_view_to_namespace_by_prepend_extender()
    {
        $this->extend(
            (new Extend\View)
                ->extendNamespace('duroom', dirname(__FILE__, 3).'/fixtures/views')
        );

        $this->assertEquals('<html><body>Hello World!</body></html>', trim($this->app()->getContainer()->make(Factory::class)->make('duroom::test')->render()));
    }
    ...
    public function can_override_view_in_namespace_by_prepend_extender()
    {
        $this->extend(
            (new Extend\View)
                ->extendNamespace('duroom', dirname(__FILE__, 3).'/fixtures/views/override')
        );
