...
use DuRoom\Extension\Event\Enabling;
use DuRoom\Extension\Event\Uninstalled;
...
class ExtensionManager
{
    ...
    public function getExtensions()
    {
        if (is_null($this->extensions) && $this->filesystem->exists($this->paths->vendor.'/composer/installed.json')) {
            ...
            // We calculate and store a set of composer package names for all installed DuRoom extensions,
            // so we know what is and isn't a duroom extension in `calculateDependencies`.
            ...
            foreach ($installed as $package) {
                if (Arr::get($package, 'type') != 'duroom-extension' || empty(Arr::get($package, 'name'))) {
                    continue;
                }
    ...
    {
        return resolve('filesystem')->disk('duroom-assets');
    }
    ...
    /**
     * Call on all enabled extensions to extend the DuRoom application.
     *
     ...
     *
     * @return array with 2 keys: 'valid' points to an ordered array of \DuRoom\Extension\Extension
