...
class ErrorHandling implements ExtenderInterface
{
     ...
     * itself (if it implements {@see \DuRoom\Foundation\KnownError}), or
     ...
     * the exception implement our {@see \DuRoom\Foundation\KnownError}
     ...
     *
     * When DuRoom's default error handling is not enough for you, and the other
     ...
     * returns a {@see \DuRoom\Foundation\ErrorHandling\HandledError} instance.
     ...
     *
     * Reporters will be called whenever DuRoom encounters an exception that it
     ...
     * {@see \DuRoom\Foundation\ErrorHandling\Reporter} interface.
     *
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        if (count($this->statuses)) {
            $container->extend('duroom.error.statuses', function ($statuses) {
                return array_merge($statuses, $this->statuses);
            });
        }

        if (count($this->types)) {
            $container->extend('duroom.error.classes', function ($types) {
                return array_merge($types, $this->types);
            });
        }

        if (count($this->handlers)) {
            $container->extend('duroom.error.handlers', function ($handlers) {
