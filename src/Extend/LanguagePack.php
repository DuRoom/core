...
class LanguagePack implements ExtenderInterface, LifecycleInterface
{
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        ...
        $locale = $extension->composerJsonAttribute('extra.duroom-locale.code');
        $title = $extension->composerJsonAttribute('extra.duroom-locale.title');

        if (! isset($locale, $title)) {
            throw new RuntimeException(
                'Language packs must define "extra.duroom-locale.code" and "extra.duroom-locale.title" in composer.json.'
            );
        }
    ...
    {
        $container->make('duroom.locales')->clearCache();
    }
