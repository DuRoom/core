...
use DuRoom\Frontend\Frontend as ActualFrontend;
...
class Frontend implements ExtenderInterface
{
     ...
     * - \DuRoom\Frontend\Document $document
    ...
    private function registerAssets(Container $container, string $moduleName): void
    {
        ...
        $abstract = 'duroom.assets.'.$this->frontend;
            ...
            if ($this->js) {
                $assets->js(function (SourceCollector $sources) use ($moduleName) {
                    ...
                    $sources->addString(function () use ($moduleName) {
                        return "duroom.extensions['$moduleName']=module.exports;";
                    });
                });
            }
        ...
        if (! $container->bound($abstract)) {
            $container->bind($abstract, function (Container $container) {
                return $container->make('duroom.assets.factory')($this->frontend);
            });
    ...
    {
        if ($this->titleDriver) {
            $container->extend('duroom.frontend.title_driver', function ($driver, Container $container) {
