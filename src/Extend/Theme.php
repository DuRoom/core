...
class Theme implements ExtenderInterface
{
     ...
     * @param string|null $extensionId : If overriding an extension file, specify its ID, for example: `duroom-tags`.
     * @return self
     */
     ...
     * ```php
     * (new Extend\Theme)
     *     ->addCustomLessFunction('is-duroom', function (mixed $text) {
     *         return strtolower($text) === 'duroom'
     *     }),
     * ```
     *
    ...
    {
        $container->extend('duroom.frontend.custom_less_functions', function (array $customFunctions) {
            return array_merge($customFunctions, $this->customFunctions);
        });

        $container->extend('duroom.assets.factory', function (callable $factory) {
