...
class FilesystemServiceProvider extends AbstractServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->container->singleton('duroom.filesystem.disks', function () {
            return [
                'duroom-assets' => function (Paths $paths, UrlGenerator $url) {
                    return [
                        'root'   => "$paths->public/assets",
                        'url'    => $url->to('forum')->path('assets')
                    ];
                },
                'duroom-avatars' => function (Paths $paths, UrlGenerator $url) {
                    return [
        ...
        $this->container->singleton('duroom.filesystem.drivers', function () {
            return [];
        });

        $this->container->singleton('duroom.filesystem.resolved_drivers', function (Container $container) {
            return array_map(function ($driverClass) use ($container) {
                return $container->make($driverClass);
            }, $container->make('duroom.filesystem.drivers'));
        });

        $this->container->singleton('filesystem', function (Container $container) {
            return new FilesystemManager(
                $container,
                $container->make('duroom.filesystem.disks'),
                $container->make('duroom.filesystem.resolved_drivers')
            );
        });
