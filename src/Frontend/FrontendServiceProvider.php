...
use DuRoom\Frontend\Driver\BasicTitleDriver;
...
class FrontendServiceProvider extends AbstractServiceProvider
{
    ...
    {
        $this->container->singleton('duroom.assets.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $paths = $container[Paths::class];

                $assets = new Assets(
                    $name,
                    $container->make('filesystem')->disk('duroom-assets'),
                    $paths->storage,
                    null,
                    $container->make('duroom.frontend.custom_less_functions')
                );
        ...
        $this->container->singleton('duroom.frontend.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $frontend = $container->make(Frontend::class);

                $frontend->content(function (Document $document) use ($name) {
                    $document->layoutView = 'duroom::frontend.'.$name;
                });
                ...
                $frontend->content(function (Document $document) use ($container) {
                    $default_preloads = $container->make('duroom.frontend.default_preloads');
        ...
        $this->container->singleton(
            'duroom.frontend.default_preloads',
            function (Container $container) {
                $filesystem = $container->make('filesystem')->disk('duroom-assets');
        ...
        $this->container->singleton(
            'duroom.frontend.custom_less_functions',
            function (Container $container) {
        ...
        $this->container->alias(TitleDriverInterface::class, 'duroom.frontend.title_driver');

        $this->container->singleton('duroom.less.config', function (Container $container) {
            return [
    ...
    {
        $this->loadViewsFrom(__DIR__.'/../../views', 'duroom');
    ...
    private function addLessVariables(SourceCollector $sources)
    {
        $sources->addString(function () {
            $vars = $this->container->make('duroom.less.config');
